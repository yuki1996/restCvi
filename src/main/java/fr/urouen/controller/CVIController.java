...
public class CVIController {
	...
	public String getListId (@PathVariable("id") int id) {
		if (id < 2) {
			...
			return new CVS("bombadil","tom", "stage", "12/12/2014").toString();
		}
	}*/
	...
	@RequestMapping(value = "/resume/{id}",
            ...
            produces=MediaType.APPLICATION_XML_VALUE)
	@ResponseBody
	//public ResponseEntity<String> getResumeId(@PathVariable("id") int id) {
	public String getResumeId(@PathVariable("id") int id) {
		...
		/*if (lescvs.getNbCV() == 0 || lescvs.getCV(id) == null) s{
			return new ResponseEntity<String>(lescvs.getCV(id), 
				      new HttpHeaders(), 
				      HttpStatus.ERROR);
		}*/
	}
	...
	@ResponseBody
	public String test() {
		AddFormPage formulaire = new AddFormPage();
    ...
    public String insert(
            ...
            @RequestParam("obj") String objectif,
        ...
        /*CVS cv = new CVS(lastname, firstname, objectif, "");
        lescvs.insert(cv);
        if (lescvs.getNbCV() == 0) {
            return "<error>Aucun cv disponible</error>";
        }*/
        String result ="<datedeb>";
        for (String date : dateDeb_certif) {
			result += date.toString();
		}
        result += "</datedeb>";
        result += "<datefin>";
        for (String date : dateFin_certif) {
			result += date.toString();
		}
        result += "</datefin>";
        result += "<descript>";
        for (String date : descript_certif) {
			result += date.toString();
		}
        result += "</descript>";
        return result;
	...
	public String insert(
		...
		CVS cv = ValidateForm.verifFormulaire(lastname, firstname, objectif, objectifType, dateDeb_exp, descript_exp,
